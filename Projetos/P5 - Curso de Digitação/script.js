const textos = [
    ...
    "Sob o céu estrelado da noite, os animais da floresta começam a se recolher em suas tocas, enquanto os grilos cantam sua sinfonia noturna, acompanhados pelo suave murmúrio do riacho que a serpenteia entre as pedras.",
    "No coração da cidade agitada, o trânsito flui incessantemente, enquanto as luzes dos prédios brilhan intensamente, refletindo-se nas águas calmas do rio que corta a metrópole, criando um cenário urbano de beleza única, onde o caos e a serenidade se encontram harmoniosamente."
];

let contadorCerto = 0, contadorErrado = 0, tempoRestante = 60, textoAtual = '';
let intervaloTempo;
...
function mudarNivel(nivel){
    ...
    elementoTexto.innerHTML = textoAtual.split('').map(caractere => `<span>${caractere}</span>`).join('');
